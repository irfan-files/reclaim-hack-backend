...
const HTTP_PROVIDER = {
    ...
    createRequest(secretParams, params) {
        ...
        utils_2.logger.info({ url: params.url, path: pathname, query: searchParams.toString() });
        ...
        const secHeadersList = (0, utils_3.buildHeaders)(secHeaders);
        utils_2.logger.info({ requestLine: reqLine });
        const httpReqHeaderStr = [
            ...
            ...(0, utils_3.buildHeaders)(pubHeaders),
            ...secHeadersList,
            '\r\n',
        ].join('\r\n');
        ...
        const tokenStartIndex = (0, utils_2.findIndexInUint8Array)(data, (0, tls_1.strToUint8Array)(secHeadersStr));
        const redactions = [
            {
    ...
    getResponseRedactions(response, rawParams) {
        var _a, _b;
        const res = (0, utils_3.parseHttpResponse)(response);
        ...
        if (bodyStartIdx < 4) {
            utils_2.logger.error({ response: (0, utils_2.uint8ArrayToBinaryStr)(response) });
        ...
        const body = (0, utils_2.uint8ArrayToBinaryStr)(res.body);
        for (const rs of params.responseRedactions || []) {
            let element = body;
            let elementIdx = 0;
            let elementLength = -1;
            if (rs.xPath) {
                const { start, end } = (0, utils_3.extractHTMLElementIndex)(body, rs.xPath, !!rs.jsonPath);
                element = body.slice(start, end);
                elementIdx = start;
                elementLength = end - start;
            }
            if (rs.jsonPath) {
                const { start, end } = (0, utils_3.extractJSONValueIndex)(element, rs.jsonPath);
                element = body.slice(elementIdx + start, elementIdx + end);
                elementIdx += start;
                elementLength = end - start;
            }
            if (rs.regex) {
                const regexp = (0, utils_3.makeRegex)(rs.regex);
                const elem = element || body;
                const match = regexp.exec(elem);
                if (!(match === null || match === void 0 ? void 0 : match[0])) {
                    utils_2.logger.error({ response: (0, utils_2.uint8ArrayToBinaryStr)(res.body) });
            ...
            if (elementIdx >= 0 && elementLength > 0) {
                const from = (0, utils_3.convertResponsePosToAbsolutePos)(elementIdx, bodyStartIdx, res.chunks);
                const to = (0, utils_3.convertResponsePosToAbsolutePos)(elementIdx + elementLength, bodyStartIdx, res.chunks);
    ...
    assertValidProviderReceipt(receipt, paramsAny) {
        ...
        const req = (0, utils_2.getHttpRequestDataFromTranscript)(receipt);
        ...
        if (protocol !== 'https:') {
            utils_2.logger.error('params URL: %s', params.url);
            logTranscript();
        ...
        const serverBlocks = receipt
            ...
            .map((r) => r.message)
            .filter(b => !b.every(b => b === utils_2.REDACTION_CHAR_CODE)); // filter out fully redacted blocks
        const response = concatArrays(...serverBlocks);
        let res;
        let pureRes;
        if (secretParams) { //means we're on client doing preliminary checks
            const parsedResp = (0, utils_3.parseHttpResponse)(response); // to deal with chunked responses
            if (parsedResp.statusCode !== 200) {
                logTranscript();
                throw new Error(`Provider returned error ${parsedResp.statusCode} ${parsedResp.statusMessage}"`);
            }
            pureRes = parsedResp.body;
            res = (0, utils_2.uint8ArrayToStr)(parsedResp.body);
        }
        else {
            pureRes = response;
            res = (0, utils_2.uint8ArrayToStr)(response);
            if (!res.startsWith(OK_HTTP_HEADER)) {
                logTranscript();
                const statusRegex = (0, utils_3.makeRegex)('^HTTP\\/1.1 (\\d{3})');
                ...
                utils_2.logger.error({ res: utils_1.base64.encode(pureRes), params: paramsAny });
                throw new Error(`Response did not start with "${OK_HTTP_HEADER}" got "${res.slice(0, lineEnd)}"`);
            }
        }
        //validate server Date header if present
        const dateHeader = (0, utils_3.makeRegex)(dateHeaderRegex).exec(res);
        if ((dateHeader === null || dateHeader === void 0 ? void 0 : dateHeader.length) > 1) {
            ...
            if ((Date.now() - serverDate) > dateDiff) {
                utils_2.logger.info({ dateHeader: dateHeader[0], current: Date.now() }, 'date header is off');
        ...
        const paramBody = params.body instanceof Uint8Array
            ? params.body
            : (0, tls_1.strToUint8Array)(params.body || '');
        if (paramBody.length > 0 && !(0, utils_3.matchRedactedStrings)(paramBody, req.body)) {
            logTranscript();
        ...
        for (const { type, value, invert } of params.responseMatches || []) {
            ...
            switch (type) {
                case 'regex':
                    const regexRes = (0, utils_3.makeRegex)(value).exec(res);
                ...
                case 'contains':
                    ...
                    if (includes === inv) {
                        logTranscript();
                        utils_2.logger.error({ res: utils_1.base64.encode(pureRes) });
...
function substituteParamValues(currentParams, secretParams, ignoreMissingBodyParams) {
    ...
    let hiddenBodyParts = [];
    if (params.body) {
        const strBody = typeof params.body === 'string' ? params.body : (0, utils_2.uint8ArrayToStr)(params.body);
